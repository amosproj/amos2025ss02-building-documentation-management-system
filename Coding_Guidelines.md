...
1.  Branching Strategy
    a. `main`: The production-ready branch. Only thoroughly tested code is merged here.
    b. `dev`: The integration branch for ongoing development. Feature branches are merged here first.
    c. `feature/{short-description}`: Used for developing new features.
    d. `fix/{short-description}`: Used for bug fixes.

2. Branching Workflow
    a. New development begins on a `feature/{name}` or `fix/{name}` branch. Developers will have work rights only on their respective branch.
    b. These branches must be created **from `dev`**, not `main`???***
    c. When development is complete, create a pull request (PR) to merge into `dev`.
    d. Developers should write a meaningful description in the PR description.
    d. At least two other eyes review the pull request, both developers. In addition, product owners or scrum master can also review if one of them case they can understand the tech stack.
    
 3. PR Workflow:   
    a. Reviewers should be able to build the branch under review, and then check the target functionality, at least locally.
    b. Comments are added by reviewers, more changes are made by developer who made the request, iteratively, till all reviewers approve.
    c. After testing and code review, `dev` will be merged into `main` as part of a release.
    d. If dev build fails, the merge can be rolled back and the responsible developer should try the fix on original branch. Or if the fail is a result of integration issues on dev branch and not a result of faulty code logic on the branch being merged, it could be solved on dev branch itself.
    
4. Branch Cleanup:
    a. Personal or outdated branches must be deleted from the remote repository after merging.
    b. Only active, shared branches should exist on the remote.

5. Branch Naming Conventions
    - Use lowercase and hyphens for clarity.
    - Examples:
        - `feature/update-coding-guidelines`
        - `fix/incorrect-document-id-check`

6.  a. Test-driven development (TDD), i.e., writing the tests first (unit tests at least) and then developing the feature could be an approach, but would be best to leave it at respective developer's discretion.
...
7.  a. Pull requests should be commented with the functionality implemented (main purpose and maybe short summary of code changes), instructions for installing dependencies, any heads-up for build process.
...
8. Release Tags & Lightweight Changelogs (To be set-up)
...
9. Coding freeze for release Tuesday 8 pm
